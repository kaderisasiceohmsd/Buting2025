...
:root{
  ...
  --neon:#7EA3FF; --neon2:#a7c0ff;
  --shadow:0 14px 40px rgba(34, 51, 102, .10);
}
html, body, [data-testid="stAppViewContainer"]{
  background: radial-gradient(1200px 600px at 18% 0%, var(--bg1), #fff 60%);
}
/* hide sidebar completely */
section[data-testid="stSidebar"]{display:none !important;}
/* container spacing & max width */
section.main > div.block-container{padding-top:0.6rem; max-width:1280px;}
...
.t-main{font-weight:1000; letter-spacing:.25px; color:var(--ink); font-size:32px;}
.t-sub{color:var(--muted); font-weight:700}
.badge{
  background:var(--gold); color:#1b1b1b; border-radius:999px; padding:10px 16px;
  font-weight:900; box-shadow:0 10px 30px rgba(203,178,121,.35);
}

/* Neon glow flicker (futuristic) */
@keyframes glowPulse{
  0%{text-shadow:0 0 0 rgba(126,163,255,0);}
  40%{text-shadow:0 0 16px rgba(126,163,255,.45), 0 0 28px rgba(74,103,233,.25);}
  100%{text-shadow:0 0 0 rgba(126,163,255,0);}
}
.t-main span{color:var(--bl); animation:glowPulse 3.2s ease-in-out infinite;}

/* ===== Cards & Utilities ===== */
.card{
  background:var(--card); border:1px solid rgba(0,0,0,.05); border-radius:16px; padding:16px 18px;
  box-shadow:var(--shadow);
}
.mono{font-family: ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, "Liberation Mono", "Courier New", monospace;}
.u-row{display:flex; gap:12px; align-items:center}
.u-col{display:flex; flex-direction:column}
.u-muted{color:var(--muted)} .u-ink{color:var(--ink)} .u-sm{font-size:12px} .u-md{font-size:14px} .u-lg{font-size:18px}
.u-bold{font-weight:900} .u-pad{padding:8px} .u-gap{gap:6px}
...
.metric{display:flex; flex-direction:column; gap:6px; padding:14px; border-radius:12px;
  background:var(--soft); border:1px solid rgba(0,0,0,.06)}
.metric .k{font-size:13px;color:var(--muted);font-weight:900; letter-spacing:.2px}
.metric .v{font-size:22px;color:var(--ink);font-weight:1000}
...
.decision{display:inline-flex; gap:10px; align-items:center; padding:10px 12px; border-radius:12px; font-weight:1000}

/* ===== Chart Container (BIG) ===== */
#fchart-host{width:100%;}
.chart-card{padding:14px; border-radius:16px; background:var(--card); border:1px solid rgba(0,0,0,.06); box-shadow:var(--shadow)}
...
st.markdown(
    ...
    <div class="hero">
      <div class="title-card">
        ...
        <div class="badge">Neon Blue ✦ Gold Pulse</div>
      </div>
    </div>
    """,
...
# --- Beta/F PDF without external libs (defensif) ---
...
# F critical table (α=0.05 base) + interpolation + alpha scaling
F_CRIT_005 = {
...
def anova_oneway(groups: Dict[str, List[float]]):
    ...
    if math.isnan(F) or math.isinf(F) or F < 0: F = 10**6
...
# -------------------------------------------------------------
# 4) SESSION STATE + CONTROLS + SCORE
# -------------------------------------------------------------
ss = st.session_state
...
with st.container():
    st.markdown('<div class="card"><div class="u-row u-gap"><span class="u-bold u-lg">🛠️ Pengaturan Eksperimen</span><span class="u-muted u-md">(ubah jumlah kelompok, sampel, dan effect size)</span></div>', unsafe_allow_html=True)
    st.markdown('<div class="slwrap">', unsafe_allow_html=True)
    c1, c2 = st.columns(2)
    with c1:
    ...
    with br:
        ...
        with b1:
            apply = st.button("Terapkan & Mulai Ronde Baru 🔁", use_container_width=True)
        with b2:
            reset = st.button("Reset Skor & Ronde ♻️", use_container_width=True)
        with b3:
            st.write(f"🏆 **Skor:** {ss.score}")
...
st.markdown('<div class="card"><div class="u-row u-gap"><span class="u-bold u-lg">🧪 Data Ronde {}</span><span class="u-muted u-md">Simulasi percobaan beberapa kelompok.</span></div>'.format(ss.round), unsafe_allow_html=True)
...
# -------------------------------------------------------------
# 7) PREDIKSI USER (gameplay)
# -------------------------------------------------------------
st.markdown('<div class="card">', unsafe_allow_html=True)
st.markdown('<div class="u-row u-gap"><span class="u-bold u-lg">🎯 Prediksi Kamu</span><span class="u-muted u-md">Berdasarkan tabel & pengetahuanmu, apakah ada perbedaan rata-rata signifikan?</span></div>', unsafe_allow_html=True)
choice = st.radio("Pilih jawaban:", ["Ya, signifikan ✅", "Tidak signifikan ❌"], index=0 if ss.user_guess is None else (0 if ss.user_guess else 1), key="user_guess_radio")
lock = st.button("Kunci Jawaban & Lihat Hasil 🧪")
st.markdown('</div>', unsafe_allow_html=True)

# -------------------------------------------------------------
# 8) ANOVA CORE + HASIL + GRAFIK (muncul setelah kunci)
# -------------------------------------------------------------
if lock:
    ...
    ss.history.append({
        "round": ss.round, "guess": "Signifikan" if ss.user_guess else "Tidak",
        "truth": significant, "F": Fcalc, "Fcrit": Fcrit, "score_before": before, "score_after": ss.score
    })
    ...
    # keputusan badge
    if significant:
        decision_html = f"<div class='decision ok'>Keputusan: Tolak H₀</div><div class='u-sm u-muted'>Karena F_hit ({fmt(Fcalc,3)}) > F_krit ({fmt(Fcrit,3)}), ada bukti perbedaan rata-rata.</div>"
    else:
        decision_html = f"<div class='decision warn'>Keputusan: Gagal Menolak H₀</div><div class='u-sm u-muted'>Karena F_hit ({fmt(Fcalc,3)}) ≤ F_krit ({fmt(Fcrit,3)}), belum cukup bukti perbedaan rata-rata.</div>"

    # feedback skor
    fb = "✅ Jawaban BENAR · +10" if correct else "❌ Jawaban SALAH · -5"
    st.markdown(f"{decision_html}<div class='u-sm u-muted'>Skor: {before} → <b>{ss.score}</b> · {fb}</div>", unsafe_allow_html=True)

    # konfeti kanan 2 detik (hanya jika signifikan)
    if significant:
        components.html(
            """
            ...
            (function(){
              ...
              for(let i=0;i<160;i++){
                parts.push({
                  ...
                  y: -20-80*Math.random(),
                  vx: 70+140*Math.random(),
                  vy: 50+90*Math.random(),
                  g: 220+Math.random()*160,
                  s: 4+Math.random()*6,
                  rot: Math.random()*6.28,
                  col: i%3? '#DCCCA3' : (i%2? '#4A67E9':'#7EA3FF')
                });
              }
              function step(ts){
    ...
    st.markdown('</div>', unsafe_allow_html=True)  # end hasil card

    # ---------------------------------------------------------
    # 8b) GRAFIK BESAR DI BAWAH (selalu utuh, auto-scale)
    # ---------------------------------------------------------
    def render_big_f_chart(df1:int, df2:int, Fh:float, Fc:float, alpha:float):
        # Domain X adaptif dengan buffer agar tidak mentok kanan
        xmax = max(Fc*1.6, Fh*1.12, 8.0 + df1)
        xmin = 0.0
        # Sample points cukup rapat untuk animasi halus
        N = 720
        ...
        <div class="chart-card fade">
          <div class="u-row u-gap u-bold" style="margin:4px 4px 10px 4px;">
            <span>📈 Visualisasi Distribusi F (α = {alpha}) — Cinematic</span>
          </div>
        ...
        (function(){{
          ...
          function draw(){{
            ...
            defs.innerHTML=`<linearGradient id='gL'><stop offset='0%' stop-color='#9db4ff' stop-opacity='0.95'/><stop offset='100%' stop-color='#c9d6ff' stop-opacity='0.3'/></linearGradient>
                            ...
                            <filter id='glow'><feGaussianBlur stdDeviation='2.3' result='b'/><feMerge><feMergeNode in='b'/><feMergeNode in='SourceGraphic'/></feMerge></filter>`;
            ...
            curve.setAttribute('fill','none'); curve.setAttribute('stroke','#4A67E9'); curve.setAttribute('stroke-width','2.9'); curve.setAttribute('filter','url(#glow)');
            ...
            vline(FcX,'#111827',1.7); vline(FhX,'#4A67E9',2.6);
    ...
    # tampilkan grafik besar di bawah hasil
...
with st.expander("🏅 Riwayat Ronde (Sesi Ini)"):
    if not ss.history:
        ...
        rows = []
        for h in ss.history:
            right = (h["guess"]=="Signifikan" and h["truth"]) or (h["guess"]=="Tidak" and not h["truth"])
            ok = "✅" if right else "❌"
            rows.append(
                f"<tr><td>{h['round']}</td><td>{h['guess']}</td>"
...
# -------------------------------------------------------------
st.caption(
    "Catatan: Tabel F-kritikal berbasis α=0.05 dengan interpolasi df2; "
    "α lain diskalakan monotonic (0.10≈−15%, 0.01≈+35%). Cukup akurat untuk edukasi & gameplay. "
    "Visualisasi F-curve digambar secara animatif; domain otomatis mengikuti F_hit/F_krit supaya tidak kepotong."
)
